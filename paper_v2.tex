...
Many molecular systems of practical interest cannot be properly described
by a single electronic configuration. Among those systems which are
often described as strongly correlated systems, we can mention transition
metal clusters that are found in many protein active sites and that
play a key role in a wide number of important biological processes
such as photosynthesis or respiration\cite{beinert_iron-sulfur_1997}.
Unfortunately because of the multideterminental nature of the wave-function
they can prove hard to study with single-reference approaches such
as the widely used density functional theory\cite{neese_critical_2006}.

Alternatively methods that deal with the multireference character
of the wavefunction can be used. The idea is to express the ground
state wavefunction as a linear combination of several configurations,
often referred as excitation from a reference determinant, usually
the Hartree-Fock one. The Full Configuration Interaction (FCI) consists
in an exact solution of the Schrodinger equation in the space of all
the possible determinants, this method is exact but become quickly
unpractical, indeed the largest space that has been handled to this
day contains $10^{10}$ determinants and were describing a $N_{2}$
molecule with 14 electrons and 36 molecular orbitals\cite{rossi_full-configuration_1999}.
To overcome this limitation the calculation can be carried by considering
only a meaningful subset of the available configurations, the most
popular choice is to use a complete active space (CAS) wave function\cite{siegbahn_comparison_1980,roos_complete_1980,siegbahn_complete_1981},
in which the molecular orbitals are split into three sets: the \textit{core}
orbitals, the \textit{virtual} orbitals and the \textit{active} orbitals.
The core and virtual orbitals are respectively doubly occupied and
empty in all configuration of the multi configuration expansion while
the active orbitals can be empty, singly or doubly occupied. This
approach allows to tackle bigger systems than the FCI one however
it suffers for the same exponential scaling problem, the biggest active
space that is possible to treat with this conventional implementation
of this approach are typical 18 orbitals and 18 electrons. Different
...
Recently two approaches have been proposed that allows to tackle large
actives spaces, typically up to 40-50 orbitals, DMRG\cite{white_density_1992,white_density-matrix_1993,white_ab_1999,wouters_chemps2:_2014,zgid_spin_2008,moritz_convergence_2005,legeza_optimizing_2003,kurashige_high-performance_2009,olivares-amaya_ab-initio_2015}
and FCIQMC\cite{booth_fermion_2009,cleland_taming_2012,booth_linear-scaling_2013,booth_towards_2013,petruzielo_semistochastic_2012}.
The first one express the wavefunction as a matrix product state (MPS)
that is optimized variationally thanks to the sweep algorithm. The
latter one is based on a stochastic propagation of walkers population
according to a set of stochastic rules that aims to mimic the propagation
of the imaginary time Schrodinger equation (ITSE) in the Fock space,
that is the space of the Slater determinants. This method, and its
\textit{initiator approximation} extension \textit{i}-FCIQMC have
proved to converge to FCI precision energies. One of the main interest
of the method is that the relevant determinants of the wavefunction
emerge spontaneously from the dynamic, preventing to have to store
a huge amount of non relevant information allowing to save memory
with respect to deterministic techniques such as FCI. Moreover the
processes governing the dynamic of a walker lying on a determinant
is independent of the other determinants occupation making the FCIQMC
algorithm almost trivially parallel. 

However the low lying occupied core orbitals and the more energetic
empty virtual orbitals that are neglected in the CAS calculation are
responsible for the so called weak or dynamical correlation. This
remaining correlation effect is important for a quantitative description
of chemical properties such as electronic excitation energies. It
is generally recovered by the use of perturbation theories which in
the case of a CAS calculation have to be compatible with a multireference
zero order wavefunction. There exist a huge variety of such multireference
multibody pertrubation theories among which we can mention CASPT2\cite{andersson_second-order_1990,andersson_secondorder_1992}
and NEVPT2\cite{angeli_introduction_2001,angeli_n-electron_2002}
or the recently propose multireference linearized coupled cluster
theory (MRLCC)\cite{sharma_multireference_2015}. In the perspective
of dealing with large systems it is worth noticing that a second-order
perturbation theory approach based on the generalized active space
self-consistent-field has recently been proposed\cite{ma_second-order_2016}. 

All the previously mentioned multireference perturbation theories
are involving a deterministic resolution of the perturbation equations
and the cost of the calculation of the perturbation by itself quickly
become intractable with the number of core and virtual orbitals, usually
this is dealt by making a further approximation known as internal
contraction, which comes at the cost of knowing the reduced density
matrices (RDM) of the active space till fourth body. 
...
the use of internal contraction, in a similar way that FCIQMC circumvent
the FCI limitations. Surprisingly only a few attempts to implement
a stochastic resolution of perturbation theories have been proposed,
among which we can mention the stochastic evaluation of MP2 energies
by Hirata and collaborators, MC-MP2. This approach is based on the
rewriting of second-order perturbation equations into the sum of two
13-dimensional integrals thanks to Laplace transform. Those integrals
are then evaluated through Monte-Carlo integration\cite{willow_stochastic_2012,willow_convergence_2013,willow_stochastic_2013}.
...
and then to stochastically sample thos graphs\cite{thom_stochastic_2007}.
...
In this article, in analogy to FCIQMC, we propose to introduce a new
population of walkers lying on determinants of the Fock space that
is propagated according to a series of stochastic processes that mimic
the projection equations of multireference perturbation theories (MRPT).
In principle this method can be applied to a any flavor of multireference
perturbation theories, but we will focus on MRLCC because the definition
of the zero order Hamiltonian and the perturbation is particularly
well suited to the FCIQMC framework. Moreover it has proved to give
better results than NEVPT2 or CASPT2\cite{sharma_multireference_2015},
the resulting method is named QMC-LCC.

In this method the sampling of zeroth and first order wavefunctions
are done simultaneously. The population on the zero order wavefunction
follows the standard FCIQMC rules while population dynamic of the
response wavefunction contains a source term that depends on the population
of the zero order wavefunction. This is practically done by allowing
walkers on one replica to spawn new walkers on another replica. 
...
Replica 1 that was initially empty starts get populated, the walkers
...
is updated by applying the $\left(\mathbb{I}-\Delta\tau\left(\hat{H_{0}}-E_{0}\mathbb{I}\right)\right)$
...
\begin{figure}
\centering{}\includegraphics[width=0.4\textwidth]{fig/fig1}\caption{Number of walkers in replica 0 (black), and in replica 1 with an initiator
...
linearly with the perturbation $\hat{V}$. \textcolor{blue}{We start
...
strictly scales linearly with the value of $\alpha$ \textcolor{blue}{because
...
here, instead we use a replica trick. This requires the additon a
...
\begin{figure}
\centering{}\includegraphics[width=0.4\textwidth]{fig/fig2}\caption{Comparison of the E2 energy with respect to the number of walkers.
...
\begin{figure}
\centering{}\includegraphics[width=0.4\textwidth]{fig/fig3}\caption{Comparison of the number of walkers that is necessary to use on replica
